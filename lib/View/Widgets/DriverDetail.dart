...
class _DriverDetailState extends State<DriverDetail> {
  ...
  Future<void> _loadTransactions() async {
    try {
      QuerySnapshot snapshot = await FirebaseFirestore.instance
          ...
          .where('driverName',
              isEqualTo: widget.tripData['driverName']) // filter by driverName
          .get();
      ...
      setState(() {
        ...
        _transactions.addAll(snapshot.docs.map((doc) {
          ...
          transaction['id'] =
              doc.id; // Add Firestore document ID to each transaction
          return transaction;
        }));
    ...
    try {
      ...
      setState(() {
        _transactions.removeWhere((transaction) =>
            transaction['id'] == transactionId); // Match using 'id'
      });
    } catch (e) {
  ...
  void _calculateTotals() {
    ...
    _totalCollected =
        _totalDriverGot - _totalDriverGave; // Calculate total collected
  }

  // Add a new transaction to Firestore
  Future<void> _addTransaction(String transactionType, String description,
      String amount, String date, String paymentMethod) async {
    try {
  ...
  Future<void> _settleTransactions(String date, String paymentMethod) async {
    try {
      ...
      await FirebaseFirestore.instance.collection('drivertransactions').add({
        'driverName':
            widget.tripData['driverName'], // Use the driver name from tripData
            ...
            TextButton(
              onPressed: () async {
                ...
                if (_dateController.text.isNotEmpty &&
                  ...
                  await _settleTransactions(_dateController.text,
                      _paymentMethod); // Call the function to settle transactions
                  Navigator.of(context).pop(); // Close the dialog
                } else {
